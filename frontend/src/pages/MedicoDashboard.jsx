...
import { Stethoscope, Play, Send, User, Clock, Eye, X, History, CheckCircle, Printer, AlertCircle, LayoutDashboard } from "lucide-react";

export default function MedicoDashboard({ user }) {
  const [fila, setFila] = useState([]);
  const [historico, setHistorico] = useState([]);
  const [abaAtiva, setAbaAtiva] = useState("espera"); // 'espera', 'geral', 'historico' ou 'consulta'
  ...
  const carregarDados = async () => {
    try {
      ...
      // Histórico apenas deste hospital
  ...
  const formatarEstado = (estadoFila, estadoTriagem) => {
      // Se não há estado na fila, ele ainda está na triagem/secretaria
      if (!estadoFila || estadoTriagem === 'pendente' || estadoTriagem === 'checkin_feito') {
        return <span className="text-amber-500 font-bold bg-amber-50 px-3 py-1 rounded-full text-xs">A Aguardar Check-in</span>;
      }
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-[2.5rem] border overflow-hidden shadow-sm animate-in fade-in duration-300">
          ...
          <table className="w-full text-left">
              <thead className="bg-white text-[10px] font-black uppercase text-slate-400 border-b">
                  <tr><th className="p-6">Prioridade</th><th className="p-6">Paciente</th><th className="p-6 text-right">Estado Atual</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                  {listaGeral.map((p, i) => (
                      <tr key={i} className="hover:bg-slate-50 transition-colors">
      ...
      {abaAtiva === "historico" && (
        <div className="bg-white rounded-[2.5rem] border overflow-hidden shadow-sm animate-in fade-in duration-300">
          ...
          <table className="w-full text-left">
              <thead className="bg-white text-[10px] font-black uppercase text-slate-400 border-b">
                <tr><th className="p-6">Data</th><th className="p-6">Paciente</th><th className="p-6 text-right">Ação</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                  {historico.map((h, i) => (
                      <tr key={i} className="hover:bg-slate-50 transition-colors">
